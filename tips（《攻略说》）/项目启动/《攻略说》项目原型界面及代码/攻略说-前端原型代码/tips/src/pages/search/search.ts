  ...
  constructor(public navCtrl: NavController, public navParams: NavParams,
    public http:HttpClient) {
      for(var i=0;i<this.arrfood.length;i++){
        this.http.get('http://192.168.56.144:8080/'+this.arrfood[i],{
      ...
      for(var i=0;i<this.arrsweet.length;i++){
        this.http.get('http://192.168.56.144:8080/'+this.arrsweet[i],{
          headers:new HttpHeaders({}),
          }).subscribe((data)=>{
      ...
      for(var i=0;i<this.arrtrip.length;i++){
        this.http.get('http://192.168.56.144:8080/'+this.arrtrip[i],{
          headers:new HttpHeaders({}),
          }).subscribe((data)=>{
      ...
      for(var i=0;i<this.arrmovies.length;i++){
        this.http.get('http://192.168.56.144:8080/'+this.arrmovies[i],{
          headers:new HttpHeaders({}),
          }).subscribe((data)=>{
