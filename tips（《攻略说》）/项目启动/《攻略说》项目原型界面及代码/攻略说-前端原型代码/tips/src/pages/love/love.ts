...
export class LovePage {
  ...
  constructor(public alertCtrl: AlertController,private http: HttpClient,public navCtrl: NavController, public navParams: NavParams) {
    ...
    for(var j in this.caixi){
      this.http.get('http://192.168.56.144:8080/'+this.caixi[j],{
        headers:new HttpHeaders({
        }),
      }).subscribe((data) => { // 监听
        for(var j in data){
...
//////////////////////////////获取数据库所有电影信息///////////////////////////////////////////////////
      for(var x=1;x<4;x++){
        this.http.get('http://192.168.56.144:8080/'+'movies'+x,{
          headers:new HttpHeaders({
          }),
        }).subscribe((data) => { // 监听
          for(var j in data){
...
////////////////////////////////////////////////////////////////////////////////////
    ...
    this.http.get('http://192.168.56.144:8080/'+this.splace[i],{
      headers:new HttpHeaders({
      }),
    }).subscribe((data) => { // 监听
      for(var i in data){
...
shoucang(e,index){
  ...
  this.http.delete('http://192.168.56.144:8080/'+'shoucang'+'/id/'+this.userid,
...
/////////////////////////////////////////////////////////////////////////////////////////////
 this.jingdian.splice(index,1);
  ...
  for(var j in this.loveJingdian){
    ...
    obj.id = this.userid;
    this.http.post('http://192.168.56.144:8080/'+'shoucang',obj,
      		{headers:this.headers}).subscribe((data) => {});
  }
}
/////////////////////////////////////////////////////////////////////////////////////////////
shoucang_food(e,index){
this.http.delete('http://192.168.56.144:8080/'+'shoucang_food'+'/id/'+this.userid,
        {headers:this.headers}).subscribe(data => {console.log('5')});
  ...
  for(var j in this.foods){
    ...
    obj.id = this.userid;
    this.http.post('http://192.168.56.144:8080/'+'shoucang_food',obj,
      		{headers:this.headers}).subscribe((data) => {});
  }
}
////////////////////////////////////////////////////////////////////////////////////////////
shoucang_movie(e,index){
  this.http.delete('http://192.168.56.144:8080/'+'shoucang_movie'+'/id/'+this.userid,
          {headers:this.headers}).subscribe(data => {console.log('5')});
    ...
    for(var j in this.movies){
      ...
      obj.id = this.userid;
      this.http.post('http://192.168.56.144:8080/'+'shoucang_movie',obj,
            {headers:this.headers}).subscribe((data) => {});
    }
    const alert = this.alertCtrl.create({
      title: 'Hi Friend!',
